...
export default function FortuneWheel({ onWin, className = '' }: FortuneWheelProps) {
  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      ...
      try {
        ...
        setupScene(app, { background: bg, pointer: ptr, button: btn });
      } catch (error) {
        console.error('Failed to load assets:', error);
        setupSceneWithoutAssets(app);
      }
    };
  ...
  const setupSceneWithoutAssets = (app: PIXI.Application) => {
    const root = new PIXI.Container();
    app.stage.addChild(root);
    ...
    button.beginPath();
    button.roundRect(-60, -25, 120, 50, 10);
    ...
    button.position.set(GAME_CONFIG.CENTER.X, GAME_CONFIG.CENTER.Y + 150);
    button.eventMode = 'static';
    button.cursor = 'pointer';
    button.on('pointerdown', () => startSpin());
