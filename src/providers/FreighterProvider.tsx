...
const FreighterContext = createContext<FreighterContextType | undefined>(
  undefined
);
...
export function FreighterProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (isConn) {
        ...
        if (networkResult.error) {
          setError(
            networkResult.error.message || "Failed to get network information"
          );
        } else {
      ...
      try {
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => {
            reject(
              new Error(
                "Connection timeout. Freighter wallet may not be installed."
              )
            );
          }, 5000);
        });
        ...
        if (
          ...
          const errorMsg =
            accessResult.error.message || accessResult.error.name;
  ...
  const handleSignAuthEntry = useCallback(
    async (
      ...
      try {
        ...
        if (result.error) {
          const errorMessage =
            result.error.message || "Failed to sign auth entry";
          setError(errorMessage);
          throw new Error(errorMessage);
        }
        return {
  ...
  const handleGetNetwork = useCallback(async () => {
    try {
      ...
      if (result.error) {
        const errorMessage =
          result.error.message || "Failed to get network information";
        setError(errorMessage);
        throw new Error(errorMessage);
      }
  ...
  const handleGetNetworkDetails = useCallback(async () => {
    try {
      ...
      if (result.error) {
        const errorMessage =
          result.error.message || "Failed to get network details";
        setError(errorMessage);
        throw new Error(errorMessage);
      }
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to get network details";
      setError(errorMessage);
      throw err;
    }
  }, []);
